...
class word2vec_game():
    ...
    def __init__(self):
        ...
        self.model = word2vec.Word2Vec.load("./data/wiki_neologd.model") # load word2vec model
    ...
    def out_result(self):
        ...
        # 類似度ランキングを計算
        top12 = self.model.most_similar(positive=self.pos,negative=self.neg,topn=self.topn)
    ...
    def pre_target(self):        
        ...
        num_min = 3
        num_max = 5
        words_cut = [word for word in words if (p.fullmatch(word) or p2.fullmatch(word)) if (len(word) >=num_min and len(word) <= num_max)]

        self.target_words = words_cut # ターゲット単語リスト => ランダムに選択
        
        # min, max (後半特にわからないのがあるので5で割った数), size
        self.target_index = np.random.randint(0, len(words_cut) // 5, self.target_num).tolist()
        return 'dummy'
    ...
    def target_out(self):
        ...
        while True:
            target1 = self.target_words[self.target_index.pop(0)] # ターゲット1 (開始単語) を取得
        ...
        while True:
            ...
            target2 = self.model.most_similar(positive=[target1],topn=topn)[random.randint(5, topn) - 1]
    ...
    def main(self, requests):
        ...
        try:
            ...
            res = {'word_check' : 'NG','target_check' : 'OK','eq' : 'EQ(%d回): '%(self.count) + self.current_str}
            return json.dumps(res, ensure_ascii=False)
